...
layui.define(['layer','element','table'], function(exports){
    ...
    table.render({
        elem: '#templateTable'
        ,url: './assets/mockTemplateData.json' //数据接口
        ,width: 948
        /*定义发送请求的参数*/
        /*,request: {
            pageName: 'page' //页码的参数名称，默认：page
        ...
        ,cols: [[ //表头
            {field: 'numbers', title: '序号', type: 'numbers',width:110,fixed:'left'}
            ,{field: 'templateName', title: '合同模板名称'}
            ,{field: 'publishTime', title: '上传时间',sort: true}
            ,{field: 'action', title: '操作',width:150, align:'center', toolbar: '#toolBtnsTemplate', fixed: 'right'}
        ]]
        , done: function(res, curr, count){
            //如果是异步请求数据方式，res即为你接口返回的信息。
            //如果是直接赋值的方式，res即为：{data: [], count: 99} data为当前页数据、count为数据总长度
    ...
    table.on('tool(templateTable)', function(obj){
        var data = obj.data;
        if(obj.event === 'preview'){
            openLayer('购售电合同详情','template_detail_iframe.html','986','622')
           // layer.msg('预览操作purchaseSaleTable');
        }else if(obj.event === 'download'){
            layer.alert('下载文件：<br>'+ JSON.stringify(data))
        }
